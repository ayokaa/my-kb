...
- **LLM Processing**: Calls the configured Anthropic Messages API–compatible endpoint to extract tags, summaries, key facts, timelines, links, QAs, and other structured information
...
- **AI Streaming**: `ai` SDK `useChat` (frontend) + native ReadableStream (backend), Anthropic Messages API
...
Required:
- `ANTHROPIC_API_KEY` — LLM API key (used for note generation and chat)

Optional:
- `ANTHROPIC_BASE_URL` — LLM API base URL, defaults to `https://api.minimaxi.com/anthropic` (can be swapped for the official Anthropic endpoint or another compatible provider)
...
│   ├── ChatPanel.tsx           # Chat layout shell
│   ├── ChatSession.tsx         # Single-chat UI component
│   ├── InboxPanel.tsx          # Inbox review panel
│   ├── IngestPanel.tsx         # Knowledge ingestion (text/link/file/RSS)
│   ├── LogsPanel.tsx           # Log viewer (real-time push + filtering)
│   ├── MemoryPanel.tsx         # User memory panel
│   ├── NotesPanel.tsx          # Server Component (fetches initial data)
│   ├── NotesPanelClient.tsx    # Client Component (interactivity + SSE)
│   ├── RSSPanel.tsx            # RSS subscription management
│   ├── SettingsPanel.tsx       # Runtime settings panel
│   ├── TasksPanel.tsx          # Task queue status panel
...
│   ├── useKeyboardShortcuts.ts    # Keyboard shortcut handler (Ctrl+Enter)
